%% Liner Discriment Analysis for saccade order
% This script is created mainly for saccade order decoding.
...
for ep = 1:5
    for sessions = Dailysessions
        ...
        for blk = blk_num
            ...
            decodeSac.(VarName).(EpochName{epp(ep)}).classes{1,blk} = classes;
            decodeSac.(VarName).(EpochName{epp(ep)}).errRate{1,blk} = err;
            decodeSac.(VarName).(EpochName{epp(ep)}).posteriors{1,blk} = posteriors;
            decodeSac.(VarName).(EpochName{epp(ep)}).logp{1,blk} = logp;
            decodeSac.(VarName).(EpochName{epp(ep)}).coef{1,blk} = coef;                       
            decodeSac.(VarName).(EpochName{epp(ep)}).CovMatrix{1,blk} = CovMatrix;
            decodeSac.(VarName).(EpochName{epp(ep)}).ConfigNum = idx_cor_blockNum';
        end
    end
end
end
% save('Results_decodeSac_epochs.mat','decodeSac');