%% Liner Discriment Analysis for target colors
% This script is created mainly for target color decoding.
...
for ep = 1:5
    for sessions = Dailysessions
        ...
        for blk = blk_num
            ...
            %% prepare firing rate and target numbers
            ...
            decodeColor.(VarName).(EpochName{epp(ep)}).classes{1,blk} = classes;
            decodeColor.(VarName).(EpochName{epp(ep)}).errRate{1,blk} = err;
            decodeColor.(VarName).(EpochName{epp(ep)}).posteriors{1,blk} = posteriors;
            decodeColor.(VarName).(EpochName{epp(ep)}).logp{1,blk} = logp;
            decodeColor.(VarName).(EpochName{epp(ep)}).coef{1,blk} = coef;
            decodeColor.(VarName).(EpochName{epp(ep)}).CovMatrix{1,blk} = CovMatrix;
            decodeColor.(VarName).(EpochName{epp(ep)}).ConfigNum = idx_cor_blockNum';
        end
    end
end
end
% save('Results_decodeColor.mat','decodeColor');