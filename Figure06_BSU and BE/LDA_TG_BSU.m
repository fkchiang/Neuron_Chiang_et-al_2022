...
% This script is created mainly for target location decoder with BSU procedure.
...
%%%% plot section  
                ...
                decodeTG.(VarName).BSU.(EpochName{ep}).classes{1,blk}  = classes;
                decodeTG.(VarName).BSU.(EpochName{ep}).decoMtx{1,blk}  = decoMtx;
                decodeTG.(VarName).BSU.(EpochName{ep}).ACCU_SIG{1,blk} = ACCU_SIG;

                decodeTG.(VarName).cumuAVE.(EpochName{ep}).idx_informativeAVE{1,blk} = idx_informativeAVE;
            ...
            print(gcf,'-dpdf','-r300','-fillpage',sprintf('decodeTG-BSU-%s%.3d-%s',SubName,sessions,EpochName{ep}));
            pause(2);
            clf;
        end
    end
end
% save('decodeTG_BSU.mat','decodeTG');