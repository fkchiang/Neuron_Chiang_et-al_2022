...
% This script is created mainly for saccade order decoder with BE procedure.
...
%%%% plot section
                subplot(2,3,blk);
                hdesac = plot(1:nSig,ACCU_SIG(Loc_informativeAVE),'-k',(1:(nSig-1))+1,ACCU_BE_AVE,'-ok'); % ,1:72,ACCU_SIG(idx_informativeMAX),'-b',1:72,ACCU_MAX,'-ob')
                if blk == 1
                    legend(hdesac,{'SU','BE'},'location','northwest'); % ,'SIG-InfoMAX','BSE-MAX'}
                    title(sprintf('deSac-BE-%s%.3d-%s',SubName,sessions,EpochName{ep}));
                    ylabel('Accuracy(%)');
                ...
                decodeSac.(VarName).BE.(EpochName{ep}).classes{1,blk}  = classes;
                decodeSac.(VarName).BE.(EpochName{ep}).decoMtx{1,blk}  = decoMtx;
                decodeSac.(VarName).BE.(EpochName{ep}).ACCU_SIG{1,blk} = ACCU_SIG;

                decodeSac.(VarName).cumuAVE.(EpochName{ep}).Loc_informativeAVE{1,blk} = Loc_informativeAVE;
                ...
                decodeSac.(VarName).cumuAVE.(EpochName{ep}).Loc_maxACCU_AVE{1,blk}    = Loc_maxACCU_AVE;
            end
            print(gcf,'-dpdf','-r300','-fillpage',sprintf('decodeSac-BE-%s%.3d-%s',SubName,sessions,EpochName{ep}));
            pause(2);
            clf;
        end
    end
end
% save('decodeSac_BE.mat','decodeSac');