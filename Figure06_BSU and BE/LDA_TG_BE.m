...
% This script is created mainly for target location decoder with BE procedure.
% Chiang,F-K, et al, NEURON 2021
% email: fengk-kuei.chiang@mssm.edu
clear;clc;close all;
figure('paperorientation','landscape','units','normalized','position',[0.1, 0.1, 0.8, 0.8]);
...
for Subject = 1:2
    ...
    for sessions = Dailysessions
        ...
        BlockNUM = sumTable(:,3);
        idx_CorTrials = ~isnan(sumTable(:,10));
...
%%%% plot section
                ...
                subplot(2,3,blk);
                hdeTG = plot(1:nSig,ACCU_SIG(Loc_informativeAVE),'-k',(1:(nSig-1))+1,ACCU_BE_AVE,'-ok'); % ,1:72,ACCU_SIG(idx_informativeMAX),'-b',1:72,ACCU_MAX,'-ob')
                if blk == 1
                    legend(hdeTG,{'SU','BE'},'location','northwest'); % ,'SIG-InfoMAX','BSE-MAX'}
                    title(sprintf('deTG-BE-%s%.3d-%s',SubName,sessions,EpochName{ep}));
                    ylabel('Accuracy(%)');
                ...
                decodeTG.(VarName).BE.(EpochName{ep}).classes{1,blk}  = classes;
                decodeTG.(VarName).BE.(EpochName{ep}).decoMtx{1,blk}  = decoMtx;
                decodeTG.(VarName).BE.(EpochName{ep}).ACCU_SIG{1,blk} = ACCU_SIG;

                decodeTG.(VarName).cumuAVE.(EpochName{ep}).Loc_informativeAVE{1,blk} = Loc_informativeAVE;
                decodeTG.(VarName).cumuAVE.(EpochName{ep}).classes_AVE{1,blk}        = classes_AVE;
                decodeTG.(VarName).cumuAVE.(EpochName{ep}).decoMtx_AVE{1,blk}        = decoMtx_AVE;
                decodeTG.(VarName).cumuAVE.(EpochName{ep}).ACCU_AVE{1,blk}           = ACCU_AVE;
                decodeTG.(VarName).cumuAVE.(EpochName{ep}).Loc_maxACCU_AVE{1,blk}    = Loc_maxACCU_AVE;
            end
            print(gcf,'-dpdf','-r300','-fillpage',sprintf('decodeTG-BE-%s%.3d-%s',SubName,sessions,EpochName{ep}));
            pause(2);
            clf;
        end
    end
end
% save('decodeTG_BE.mat','decodeTG');