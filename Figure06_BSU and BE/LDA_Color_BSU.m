...
% This script is created mainly for target color decoder with BSU procedure.
...
%%%% plot section  
                ...
                if blk == 1
                    legend(hdesac,{'SU','BSU'},'location','northwest');
                    title(sprintf('deColor-BSU-%s%.3d-%s',SubName,sessions,EpochName{epp(ep)}));
                    ylabel('Accuracy(%)');
                ...
                decodeColor.(VarName).BSU.(EpochName{ep}).classes{1,blk}  = classes;
                decodeColor.(VarName).BSU.(EpochName{ep}).decoMtx{1,blk}  = decoMtx;
                decodeColor.(VarName).BSU.(EpochName{ep}).ACCU_SIG{1,blk} = ACCU_SIG;

                decodeColor.(VarName).cumuAVE.(EpochName{ep}).idx_informativeAVE{1,blk} = idx_informativeAVE;
                decodeColor.(VarName).cumuAVE.(EpochName{ep}).classes_AVE{1,blk}        = classes_AVE;
                decodeColor.(VarName).cumuAVE.(EpochName{ep}).decoMtx_AVE{1,blk}        = decoMtx_AVE;
                decodeColor.(VarName).cumuAVE.(EpochName{ep}).ACCU_AVE{1,blk}           = ACCU_AVE;
            end
            print(gcf,'-dpdf','-r300','-fillpage',sprintf('decodeColor-BSU-%s%.3d-%s',SubName,sessions,EpochName{epp(ep)}));
            pause(2);
            clf;
        end
    end
end
% save('decodeColor_BSU.mat','decodeColor');